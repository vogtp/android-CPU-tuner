...
public class DBBackendTimeinstateInput {
	...
	public static int delete(CpuTunerOpenHelper openHelper, Uri uri, String selection, String[] selectionArgs) {
		...
		case TIS:
			count = db.delete(DB.TimeInStateInput.TABLE_NAME, selection, selectionArgs);
			break;

		case TIS_ID:
			String id = uri.getPathSegments().get(1);
			count = db.delete(DB.TimeInStateInput.TABLE_NAME, DB.NAME_ID + "=" + id + (!TextUtils.isEmpty(selection) ? " AND (" + selection + ')' : ""),
					selectionArgs);
			break;
	...
	public static String getType(Uri uri) {
		switch (sUriMatcher.match(uri)) {
		case TIS:
			return DB.TimeInStateInput.CONTENT_TYPE;

		case TIS_ID:
			return DB.TimeInStateInput.CONTENT_ITEM_TYPE;
	...
	public static Cursor query(CpuTunerOpenHelper openHelper, Uri uri, String[] projection, String selection, String[] selectionArgs, String sortOrder) {
		...
		if (TextUtils.isEmpty(sortOrder)) {
			orderBy = DB.TimeInStateInput.SORTORDER_DEFAULT;
		} else {
			orderBy = sortOrder;
		}
	...
	public static int update(CpuTunerOpenHelper openHelper, Uri uri, ContentValues values, String selection, String[] selectionArgs) {
		...
		switch (sUriMatcher.match(uri)) {
		case TIS:
			count = db.update(DB.TimeInStateInput.TABLE_NAME, values, selection, selectionArgs);
			break;

		case TIS_ID:
			String id = uri.getPathSegments().get(1);
			count = db.update(DB.TimeInStateInput.TABLE_NAME, values, DB.NAME_ID + "=" + id + (!TextUtils.isEmpty(selection) ? " AND (" + selection + ')' : ""),
					selectionArgs);
			break;
	...
	public static Uri insert(CpuTunerOpenHelper openHelper, Uri uri, ContentValues initialValues) {
		...
		SQLiteDatabase db = openHelper.getWritableDatabase();
		long rowId = db.insert(DB.TimeInStateInput.TABLE_NAME, null, values);
		if (rowId > 0) {
			Uri retUri = ContentUris.withAppendedId(TimeInStateInput.CONTENT_URI, rowId);
			return retUri;
		}
